...
const CreateProduct = () => {
  ...
  const onChange = async (
    ...
    try {
      ...
      const result = await (ipfs as IPFSHTTPClient).add(file as File);
      const url = `https://nftstorage.link/ipfs/${result.path}`;
      console.log(`https://nftstorage.link/ipfs/${result.path}`);
      setImages([...images, url as string]);
    } catch (err) {
      console.log(err);
    }
